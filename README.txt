...
class IOHandRLEnv(gym.Env):
    def __init__(self):
        self.model_path = "io_hand_with_site.xml"
        self.mujoco_model = mujoco.MjModel.from_xml_path(self.model_path)
        self.mujoco_data = mujoco.MjData(self.mujoco_model)
        ...
        self.action_space = spaces.Box(low=-1.0, high=1.0, shape=(self.mujoco_model.nu,), dtype=np.float32)
        obs_dim = self.mujoco_model.nq + self.mujoco_model.nv + 3
        self.observation_space = spaces.Box(low=-np.inf, high=np.inf, shape=(obs_dim,), dtype=np.float32)
    ...
    def step(self, action):
        ...
        print(f"  Joint Angles (qpos): {np.round(qpos, 3)}")
        print(f"  Joint Velocities (qvel): {np.round(qvel, 3)}")
        print(f"  EE Position: {np.round(ee_pos, 3)}")
...
if __name__ == "__main__":
    ...
    model = PPO("MlpPolicy", env, verbose=1)
    model.learn(total_timesteps=100_000)
    model.save("ppo_iohand_debug")
    print("✅ Training complete with debug output.")
